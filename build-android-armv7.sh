...
LDFLAGS="-static-libgcc -L$PREFIX/lib" LIBS="-lssl -lcrypto -lz -ldl -llog -pthread" CPPFLAGS="-I$PREFIX/include" PKG_CONFIG_PATH=$PREFIX/lib/pkgconfig ./configure --host=$HOST --prefix=/php --disable-all --enable-cli --enable-cgi --enable-mbstring --enable-json --enable-filter --enable-pcre --enable-session --enable-libxml --enable-xml --with-curl=$PREFIX --with-openssl=$PREFIX --with-zlib=$PREFIX --without-sqlite3 --without-pdo --disable-phpdbg --disable-fpm --disable-maintainer-mode
